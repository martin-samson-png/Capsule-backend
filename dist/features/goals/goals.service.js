...
export class GoalsService {
    async create(input) {
        ...
        const { data, error } = await supabaseUser
            ...
            .select("id, label , target_amount_cents, current_amount_cents, deadline, status, created_at")
            .single();
        if (error)
    ...
    async getByUserId(input) {
        ...
        if (input.deadLineTo)
            deadLineToPgDate = convertToPgDate(input.deadLineTo);
        let query = supabaseUser
            .from("goals")
            .select("id, label , target_amount_cents, current_amount_cents, deadline, status, created_at")
            .eq("user_id", input.userId);
        if (input.label)
            query.eq("label", input.label);
        if (input.status)
            query.eq("status", input.status);
        if (input.deadlineFrom)
            query.gte("deadline", deadlineFromPgDate);
        if (input.deadLineTo)
            query.lte("deadline", deadLineToPgDate);
        ...
        if (error)
            throw AppError.internalServer("Erreur lors de la récupération des objectifs");
    ...
    async delete(goalId, accessToken) {
