...
router.post("/create", requireAuth, validate(transactionCreateSchema), (req, res, next) => transactionController.create(req, res, next));
router.get("/", requireAuth, validate(transactionFindSchema, "query"), (req, res, next) => transactionController.getByUserId(req, res, next));
router.get("/:id", requireAuth, validate(idParamSchema, "params"), (req, res, next) => {
    transactionController.getById(req, res, next);
});
router.patch("/:id", requireAuth, validate(transactionUpdateSchema), validate(idParamSchema, "params"), (req, res, next) => {
    transactionController.update(req, res, next);
});
router.delete("/:id", requireAuth, validate(idParamSchema, "params"), (req, res, next) => {
